...
require_once __DIR__ . '/../../generate-student-id.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($enrollmentId > 0 && in_array($action, ['approve', 'reject', 'missing_documents'])) {
        ...
        $stmt = $conn->prepare("SELECT e.*, s.student_id as current_student_id, s.id as student_table_id, s.program FROM enrollments e JOIN students s ON e.student_id = s.student_id WHERE e.id = ?");
...
$stmt = $conn->prepare("SELECT e.*, CONCAT(s.first_name, ' ', s.last_name) as student_name, s.email as student_email, s.contact_number, s.student_id as current_student_id FROM enrollments e LEFT JOIN students s ON e.student_id = s.student_id WHERE e.status = 'pending' ORDER BY e.date_submitted DESC");
$stmt->execute();

...
$sql = "SELECT * FROM enrollments WHERE status = 'Pending'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_enrollment'])) {
    ...
    $update_sql = "UPDATE enrollments SET status = 'Approved', student_id = ? WHERE id = ?";
    $stmt = $conn->prepare($update_sql);
    $stmt->bind_param("si", $student_id, $enrollment_id);

    if ($stmt->execute()) {
        echo "<script>alert('Enrollment approved successfully.'); window.location.href='pending-enrollments.php';</script>";
    } else {
        echo "<script>alert('Error approving enrollment.');</script>";
    }
}
?>

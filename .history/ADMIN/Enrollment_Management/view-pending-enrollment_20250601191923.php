...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'approve') {
            ...
            if ($enrollments) {
                // Insert into approved_enrollments
                $insert = $pdo->prepare("
                    INSERT INTO approved_enrollments 
                    (student_id, status, semester, school_year, program, year_level, rejection_reason, section, remarks, notification, date_submitted)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $insert->execute([
                    ...
                    $enrollments['rejection_reason'],
                    $enrollments['section'],
                    $enrollments['remarks'],
                    $enrollments['notification'],
                    $enrollments['date_submitted']
                ]);

                // Delete from original enrollments

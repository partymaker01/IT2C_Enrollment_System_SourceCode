    ...
    try {
        switch ($action) {
            case 'approve':
                ...
                $stmt->execute();
                $response = ['success' => true, 'message' => 'Enrollment approved successfully!'];
                break;
        ...
        function processEnrollment(enrollmentId, action, studentName) {
            ...
            switch(action) {
                case 'approve':
                    ...
                    modalMessage.textContent = `Are you sure you want to approve the enrollment for ${studentName}?`;

...
// Enhanced fetchOptions function with better error handling
function fetchOptions($conn, $column, $table = 'students') {
    ...
    $sql = "SELECT DISTINCT $column FROM $table WHERE $column IS NOT NULL AND $column != '' AND $column != 'NULL' ORDER BY $column ASC";
...
// Handle export requests (same as original)
    ...
    <div class="main-container">
        ...
        <?php if ($debug_mode): ?>
            <div class="debug-info">
                ...
                <p><strong>Programs found:</strong> <?= count($programOptions) ?> (<?= implode(', ', array_slice($programOptions, 0, 5)) ?>)</p>
                <p><strong>Year levels found:</strong> <?= count($yearLevelOptions) ?> (<?= implode(', ', $yearLevelOptions) ?>)</p>
                <p><strong>Sections found:</strong> <?= count($sectionOptions) ?> (<?= implode(', ', $sectionOptions) ?>)</p>
                <p><strong>School years found:</strong> <?= count($schoolYearOptions) ?> (<?= implode(', ', $schoolYearOptions) ?>)</p>
            </div>
        <?php endif; ?>

        <?php if (empty($students) && !$hasFilter): ?>
            <div class="alert alert-warning" role="alert">
                <i class="bi bi-exclamation-triangle"></i>
                <strong>No student data found!</strong> 
                <p class="mb-0 mt-2">This could mean:</p>
                <ul class="mb-0">
                    <li>No students have been added to the system yet</li>
                    <li>The database columns might have different names</li>
                    <li>There might be a database connection issue</li>
                </ul>
                <p class="mt-2 mb-0">
                    <a href="?debug=1" class="btn btn-sm btn-outline-warning">Enable Debug Mode</a>
                </p>
            </div>
        <?php endif; ?>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="8" class="text-center py-4">
                                    ...
                                    <p class="text-muted">Try adjusting your filter criteria or check if students exist in the database.</p>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>

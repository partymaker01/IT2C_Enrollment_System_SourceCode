...
$sql = "SELECT e.id as enrollment_id, s.full_name, s.program, e.semester, e.school_year
        FROM enrollment e
        JOIN students s ON e.student_id = s.student_id
        ORDER BY e.school_year DESC, e.semester DESC, s.full_name ASC";
    ...
    <div id="enrollmentCards">
      ...
      if ($result && $result->num_rows > 0) {
          while ($enrollment = $result->fetch_assoc()) {
              ...
              $sqlSubjects = "SELECT subject_code, description, units, instructor, grade, remarks 
                              FROM enrollment_subjects WHERE enrollment_id = $enrollment_id";
              $subResult = $conn->query($sqlSubjects);

              // Prepare data attributes for filtering (lowercase for JS)
              $studentLower = strtolower($enrollment['full_name']);
              ...
              <div class="card mb-4 enrollment-card"
                   data-student="<?= htmlspecialchars($studentLower) ?>"
                   data-semester="<?= htmlspecialchars($semesterLower) ?>"
                   data-program="<?= htmlspecialchars($programLower) ?>">
                <div class="card-header bg-success text-white fw-bold">
                  <?= htmlspecialchars($enrollment['full_name']) ?> - <?= htmlspecialchars($enrollment['program']) ?> - <?= htmlspecialchars($enrollment['semester']) ?>, <?= htmlspecialchars($enrollment['school_year']) ?>
                </div>
                <div class="card-body p-0">
                  ...
                  <?php } else { ?>
                    <p class="no-subjects m-3">
                      No enrolled subjects this semester.
                    </p>
                  <?php } ?>
                </div>
              </div>

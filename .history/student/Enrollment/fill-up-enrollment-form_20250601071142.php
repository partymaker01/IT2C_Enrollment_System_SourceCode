<?php
session_start();
require_once __DIR__ . '/../../config/db.php'; // ✅ make sure this path is correct to your db.php
...
if (!isset($_SESSION['student_id'])) {
    ...
    $student_id_str = $_SESSION['student_id']; // This should be the student_number if that's what you saved

    // Query to get the `id` from `students` table using student_number
    $stmt = $conn->prepare("SELECT id FROM students WHERE student_number = ?");
    $stmt->bind_param("s", $student_id_str);
    $stmt->execute();
    $stmt->bind_result($student_id_int);
    $stmt->fetch();
    $stmt->close();

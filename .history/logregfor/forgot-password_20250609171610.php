    ...
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            ...
            if ($user) {
                ...
                // Store token in database - use both email and student_id for flexibility
                $stmt = $pdo->prepare("INSERT INTO password_resets (email, student_id, token, expires_at) VALUES (?, ?, ?, ?)");
                $stmt->execute([$email, $user['student_id'], $token, $expires]);

    ...
    if ($username && $first_name && $middle_name && $last_name && $email && $password && $password_verify) {
        if ($password !== $password_verify) {
            ...
            if ($stmt->num_rows > 0) {
              ...
              $stmt = $conn->prepare("INSERT INTO students (username, first_name, middle_name, email, password) VALUES (?, ?, ?, ?, ?)");
              $stmt->bind_param("sssss", $username, $first_name, $middle_name, $email, $hashed_password);

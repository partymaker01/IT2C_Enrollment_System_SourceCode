...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($student_id && $first_name && $last_name && $year_level && $course && $email && $raw_password) {
        $password = password_hash($raw_password, PASSWORD_DEFAULT);
        $verification_code = bin2hex(random_bytes(16));

        $check = $conn->prepare("SELECT id FROM students WHERE student_nid = ? OR email = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO students (student_id, first_name, last_name, year_level, course, email, password, verification_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssss", $student_id, $first_name, $last_name, $year_level, $course, $email, $password, $verification_code);
      ...
      <div class="col-md-6">
        <div class="card shadow p-4">
          ...
          <form method="POST" action="register.php">
            <input type="text" name="student_id" class="form-control mb-2" placeholder="Student Number" required />

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_action'] ?? '') === 'register') {
    ...
    $password_verify = $_POST['password_verify'] ?? $_POST['password'] ?? '';
    ...
    if ($username && $first_name && $middle_name && $last_name && $email && $password && $password_verify) {
        ...
        if ($password !== $password_verify) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                // Generate a unique student number (you can change this logic)
                $student_number = uniqid('SN');
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                $stmt->close();

                $stmt = $conn->prepare("INSERT INTO students (student_number, username, first_name, middle_name, last_name, email, password) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sssssss", $student_number, $username, $first_name, $middle_name, $last_name, $email, $hashed_password);
                if ($stmt->execute()) {

...
if (isset($_SESSION['student_id'])) {
    header("Location: /IT2C_Enrollment_System_SourceCode/student/student-dashboard.php");
    exit;
}
    ...
    if ($username && $first_name && $middle_name && $last_name && $email && $password && $password_verify) {
        if ($password !== $password_verify) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                $student_id = NULL; // Don't auto-generate student_id
                $student_number = uniqid('STU'); // Optional: you may still auto-gen this
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                $stmt->close();

                $stmt = $conn->prepare("INSERT INTO students (student_id, student_number, username, first_name, middle_name, last_name, email, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssssss", $student_id, $student_number, $username, $first_name, $middle_name, $last_name, $email, $hashed_password);

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_action'] ?? '') === 'register') {
    ...
    if ($username && $first_name && $middle_name && $last_name && $email && $password && $password_verify) {
        if ($password !== $password_verify) {
            ...
            $stmt = $conn->prepare("SELECT students FROM username WHERE password = ? OR email = ?");
            ...
            if ($stmt->num_rows > 0) {
              ...
              $stmt = $conn->prepare("INSERT INTO students (student_id, username, first_name, middle_name, email, password) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $student_id, $username, $first_name, $middle_name, $email, $hashed_password);
